<template>
  <a-modal v-model="show" title="预约详情" @cancel="onClose" :width="800">
    ...
    <div style="font-size: 13px;font-family: SimHei" v-if="dishesData !== null">
      <a-row style="padding-left: 24px;padding-right: 24px;">
        ...
        <a-col :span="8"><b>预约人：</b>
          {{ dishesData.studentName }}
        </a-col>
        <a-col :span="8"><b>课程日期：</b>
          {{ dishesData.courseDate ? dishesData.courseDate : '- -' }}
        </a-col>
        <a-col :span="8"><b>预约课程：</b>
      ...
      <a-row style="padding-left: 24px;padding-right: 24px;" v-if="staffList.length !== 0">
        <a-col style="margin-bottom: 15px"><span style="font-size: 15px;font-weight: 650;color: #000c17">预约邀请人</span></a-col>
        <a-row :gutter="15">
...
export default {
  ...
  watch: {
    dishesShow: function (value) {
      if (value) {
        this.imagesInit(this.dishesData.studentImages)
      }
    }
  },
  methods: {
    queryStaffListByCondition (id) {
      this.$get('/cos/course-reserve-info/queryStaffListByCondition', {conditionId: id}).then((r) => {
