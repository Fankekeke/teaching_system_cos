  ...
  <a-modal v-model="show" title="新增预约" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        ...
        <a-col :span="12">
          <a-form-item label='预约地址' v-bind="formItemLayout">
            <a-input v-decorator="[
            'address',
        ...
        <a-col :span="12">
          <a-form-item label='预约特性' v-bind="formItemLayout">
            <a-select v-decorator="[
              'status',
              { rules: [{ required: true, message: '请输入预约特性!' }] }
              ]">
        ...
        <a-col :span="24">
          <a-form-item label='预约邀请' v-bind="formItemLayout">
            <a-select mode="multiple" style="width: 100%" v-decorator="[
            'staffIdList',
            { rules: [{ required: true, message: '请输入预约邀请人!' }] }
        ...
        <a-col :span="24">
          <a-form-item label='预约内容' v-bind="formItemLayout">
            ...
            'content',
             { rules: [{ required: true, message: '请输入预约内容!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$post('/cos/course-reserve-info', {
            ...values
          }).then((r) => {
            this.reset()
            this.$emit('success')
