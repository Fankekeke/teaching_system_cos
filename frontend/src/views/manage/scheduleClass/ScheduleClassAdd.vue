<template>
  <a-modal v-model="show" title="新增班级课表" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='班级课表标题' v-bind="formItemLayout">
            <a-input v-decorator="[
            'title',
            { rules: [{ required: true, message: '请输入班级课表标题!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label='班级课表地址' v-bind="formItemLayout">
            <a-input v-decorator="[
            'address',
            { rules: [{ required: true, message: '请输入班级课表地址!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
        ...
        <a-col :span="12">
          <a-form-item label='班级课表特性' v-bind="formItemLayout">
            <a-select v-decorator="[
              'status',
              { rules: [{ required: true, message: '请输入班级课表特性!' }] }
              ]">
        ...
        <a-col :span="24">
          <a-form-item label='班级课表邀请' v-bind="formItemLayout">
            <a-select mode="multiple" style="width: 100%" v-decorator="[
            'staffIdList',
            { rules: [{ required: true, message: '请输入班级课表邀请人!' }] }
        ...
        <a-col :span="24">
          <a-form-item label='班级课表内容' v-bind="formItemLayout">
            <a-textarea :rows="6" v-decorator="[
            'content',
             { rules: [{ required: true, message: '请输入班级课表内容!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$post('/cos/schedule-class-info', {
            ...values
          }).then((r) => {
            this.reset()
            this.$emit('success')
