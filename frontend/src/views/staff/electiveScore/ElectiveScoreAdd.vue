<template>
  <a-modal v-model="show" title="新增选修分数" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='选修分数标题' v-bind="formItemLayout">
            <a-input v-decorator="[
            'title',
            { rules: [{ required: true, message: '请输入选修分数标题!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label='选修分数地址' v-bind="formItemLayout">
            <a-input v-decorator="[
            'address',
            { rules: [{ required: true, message: '请输入选修分数地址!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
        ...
        <a-col :span="12">
          <a-form-item label='选修分数特性' v-bind="formItemLayout">
            <a-select v-decorator="[
              'status',
              { rules: [{ required: true, message: '请输入选修分数特性!' }] }
              ]">
        ...
        <a-col :span="24">
          <a-form-item label='选修分数邀请' v-bind="formItemLayout">
            <a-select mode="multiple" style="width: 100%" v-decorator="[
            'staffIdList',
            { rules: [{ required: true, message: '请输入选修分数邀请人!' }] }
        ...
        <a-col :span="24">
          <a-form-item label='选修分数内容' v-bind="formItemLayout">
            <a-textarea :rows="6" v-decorator="[
            'content',
             { rules: [{ required: true, message: '请输入选修分数内容!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$post('/cos/elective-score-record', {
            ...values
          }).then((r) => {
            this.reset()
            this.$emit('success')
